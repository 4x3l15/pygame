...
FPS = 60
PLAYER_SPEED = 6
SPAWN_INTERVAL = 900   # milisegundos entre spawns
GAME_TIME = 60         # duración de la partida (segundos)
...
class Player(pygame.sprite.Sprite):
    """Jugador controlado por el usuario (bicicleta)."""
    ...
    def _draw_player(self):
        """Dibuja al jugador como una bici simple."""
        surf = self.image
        surf.fill((0,0,0,0))
        # ruedas
        pygame.draw.ellipse(surf, BLACK, (3, self.height-14, 14, 14))  
        pygame.draw.ellipse(surf, BLACK, (self.width-17, self.height-14, 14, 14))  
        # cuerpo
        pygame.draw.polygon(surf, BLUE, [(10,10),(30,10),(35,20),(20,20)])  
        # manubrio
        pygame.draw.line(surf, BLACK, (30,10),(40,5),2)  

    def update(self):
        """Actualiza la posición del jugador según teclas presionadas."""
...
class FallingObject(pygame.sprite.Sprite):
    """Objetos que caen (buenos o malos)."""
    ...
    def _draw_kind(self):
        """Dibuja el objeto según si es bueno o malo."""
        s = self.image
        s.fill((0,0,0,0))
        if self.kind == "good":
            # elegir aleatoriamente entre varios objetos buenos
            ...
            # objetos negativos (más variedad)
            choice = random.choice(["car","factory","trash","chimney"])
            if choice == "car":
                ...
                pygame.draw.rect(s, BLACK, (6, self.size-18, self.size-12, 14))
                pygame.draw.line(s, GRAY, (6, self.size-18), (self.size-6, self.size-18), 2)
            elif choice == "chimney":
                pygame.draw.rect(s, GRAY, (self.size//3, self.size//3, self.size//3, self.size-10))
                pygame.draw.circle(s, BLACK, (self.size//2, self.size//3), 8)

    def update(self):
        """Movimiento hacia abajo y autodestrucción fuera de pantalla."""
...
def save_readme():
    """Genera archivo con instrucciones del juego."""
...
- Evitar autos, fábricas, basura y chimeneas que aumentan la huella.
...
def show_start_screen(screen):
    ...
    draw_text(screen, "HUELLA DE CARBONO", 48, WIDTH//2, HEIGHT//2 - 100, center=True)
    draw_text(screen, "Mueve con ← → o A/D", 28, WIDTH//2, HEIGHT//2 - 30, center=True)
    draw_text(screen, "Evita los objetos rojos y grises", 28, WIDTH//2, HEIGHT//2 + 10, center=True)
    draw_text(screen, "Presiona ENTER para empezar", 24, WIDTH//2, HEIGHT//2 + 80, center=True)
    draw_text(screen, "Presiona ESC para salir", 20, WIDTH//2, HEIGHT//2 + 120, center=True)
    pygame.display.flip()
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == spawn_event and not paused:
                # probabilidad de objeto bueno/malo
        ...
        if not paused:
            ...
            for h in hits:
                ...
                # subir de nivel cada 10 objetos
